...
const navSections = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: '/dashboard/investments', label: 'Share Collection', icon: TrendingUp },
      { href: '/dashboard/dividends', label: 'Dividends', icon: PiggyBank },
      { href: '/dashboard/certificates', label: 'Share Certificates', icon: Award },
      { href: '/dashboard/loans', label: 'Loans', icon: Wallet },
      { href: '/dashboard/directors', label: 'Directors (BOD)', icon: Briefcase },
    ],
  },
  {
    title: 'Operations',
    links: [
      { href: '/dashboard/company-investments', label: 'Company Investments', icon: Briefcase },
      { href: '/dashboard/banks', label: 'Banking', icon: Landmark },
      { href: '/dashboard/petty-cash', label: 'Petty Cash', icon: Wallet },
      { href: '/dashboard/returns', label: 'Returns (ROI)', icon: ArrowDownToLine },
      { href: '/dashboard/expenses', label: 'Expenses', icon: Receipt },
      { href: '/dashboard/meetings', label: 'Board Meetings', icon: Calendar },
      { href: '/dashboard/chalani', label: 'Chalani (Letters)', icon: FileText },
...
export default function DashboardLayout({
  ...
  const hasPermission = (href: string) => {
    if (profile?.role === 'super_admin' || profile?.roles?.permissions?.all) return true;
  ...
  return (
    <div className="app-layout" suppressHydrationWarning>
      ...
      <main className="main-content">
        {isAllowedPath ? children : <div className="p-8 text-center text-muted">Access Denied. You do not have permissions for this module.</div>}
      </main>
    </div>
  );
}
